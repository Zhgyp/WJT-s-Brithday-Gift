...
const fruitPlacementArea = document.getElementById("fruitPlacementArea");
...
async function finishManualCream() {
    ...
    decorateTip.textContent =
        "水果是可选装饰：可以添加并拖到蛋糕顶部或侧面，也可以不加水果直接进入插蜡烛。";
    confirmDecorateBtn.hidden = false;
    confirmDecorateBtn.textContent = "不加水果，直接完成 ✓";
}
...
function updateDecorationProgress() {
    ...
    confirmDecorateBtn.hidden = decorateCompleted || !creamCompleted;
    confirmDecorateBtn.textContent = placedFruit.length > 0
        ? "确认水果装饰 ✓"
        : "不加水果，直接完成 ✓";

    if (!decorateCompleted) {
        decorateTip.textContent = placedFruit.length > 0
            ? "水果可在蛋糕顶部和侧面自由拖动；满意后确认，也可以继续添加。"
            : "水果为可选项：可以直接确认进入插蜡烛，也可以从水果篮添加。";
    }
}
...
function getFruitAllowedRects() {
    // Coordinates are relative to the fixed 560 x 440 decorate-cake model.
    const map = {
        1: [
            { x: 30, y: 178, w: 500, h: 190 }
        ],
        2: [
            { x: 30, y: 212, w: 500, h: 190 },
            { x: 95, y: 105, w: 370, h: 155 }
        ],
        3: [
            { x: 30, y: 228, w: 500, h: 190 },
            { x: 95, y: 121, w: 370, h: 155 },
            { x: 152.5, y: 18, w: 255, h: 132 }
        ]
    };
    return map[selectedCakeLayers] || map[1];
}

function clampPointToCake(x, y, element) {
    const areaRect = fruitPlacementArea.getBoundingClientRect();
    const scaleX = areaRect.width / 560;
    const scaleY = areaRect.height / 440;
    const halfWidth = Math.max(element.offsetWidth / 2, 18);
    const halfHeight = Math.max(element.offsetHeight / 2, 18);

    const rects = getFruitAllowedRects().map(function (r) {
        return {
            left: r.x * scaleX + halfWidth,
            right: (r.x + r.w) * scaleX - halfWidth,
            top: r.y * scaleY + halfHeight,
            bottom: (r.y + r.h) * scaleY - halfHeight
        };
    });

    let best = null;
    rects.forEach(function (r) {
        const px = Math.max(r.left, Math.min(r.right, x));
        const py = Math.max(r.top, Math.min(r.bottom, y));
        const dx = x - px;
        const dy = y - py;
        const distance = dx * dx + dy * dy;
        if (!best || distance < best.distance) best = { x: px, y: py, distance: distance };
    });

    return {
        x: best.x,
        y: best.y,
        xPercent: (best.x / areaRect.width) * 100,
        yPercent: (best.y / areaRect.height) * 100
    };
}
...
function enableFruitDragging(element) {
    ...
    element.addEventListener("pointermove", function (event) {
        ...
        const rect = fruitPlacementArea.getBoundingClientRect();
...
function addFruit(type, sourceButton) {
    ...
    const top = topTargets[selectedCakeLayers] || topTargets[1];
    const target = {
        x: Math.max(10, Math.min(90, top.x + (baseTarget.x - 50) * 0.45 + offsetX)),
        y: Math.max(6, Math.min(92, top.y + (baseTarget.y - 50) * 0.18 + offsetY))
    };
    ...
    fruitPlacementArea.appendChild(element);
    ...
    const cakeRect = fruitPlacementArea.getBoundingClientRect();
...
confirmDecorateBtn.addEventListener("click", async function () {
    if (decorateCompleted || !creamCompleted) {
        return;
    }
...
function syncCandleLayerHeight() {
    if (!candleScene || !candleLayer || !candleCakeMount) return;
    candleScene.classList.remove("candle-layers-1", "candle-layers-2", "candle-layers-3");
    candleScene.classList.add("candle-layers-" + selectedCakeLayers);
    candleLayer.dataset.layers = String(selectedCakeLayers);
    candleCakeMount.dataset.layers = String(selectedCakeLayers);
}

function mountFinishedCake() {
    candleCakeMount.innerHTML = "";
    syncCandleLayerHeight();
    const cakeClone = decorateCake.cloneNode(true);
    cakeClone.removeAttribute("id");
    cakeClone.classList.add("candle-finished-cake", "frosted");

    // Cream colour variables live on the decorating scene. Copy them to the
    // cloned cake so the candle scene keeps the same solid frosting instead
    // of falling back to transparent CSS custom-property values.
    const creamVariableNames = [
        "--cream-light", "--cream-mid", "--cream-dark",
        "--cream-edge", "--cream-side", "--cream-label",
        "--cream-bag-a", "--cream-bag-b", "--cream-bag-c"
    ];
    const decorateStyles = getComputedStyle(decorateSceneRoot);
    creamVariableNames.forEach(function (name) {
        const value = decorateStyles.getPropertyValue(name).trim();
        if (value) cakeClone.style.setProperty(name, value);
    });
